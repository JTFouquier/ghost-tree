...
from ghosttree.scaffold.hybridtree import scaffold_extensions_into_foundation
...
def cli():
    ...
    -> For more detailed help with each command, enter --help
    ...
    you will see the command 'ghost-tree silva extract-fungi')

    -> Arguments must be entered in order, without "-"/dashes.
...
@click.argument("output-alignment-file", type=click.File("w"))
...
def hybrid_tree(extension_trees_otu_map, extension_trees_taxonomy_file,
                extension_trees_sequence_file, foundation_alignment_file,
                ghost_tree_output_file):
    """Combines two genetic databases into
    one phylogenetic tree and is the final command in ghost-tree.
    ...
    (these are the "extension trees").

    Other databases provide high quality phylogenetic information (hence it
    is used as the "foundation"), but poor taxonomic resolution.

    The output file is one phylogenetic tree in .nwk format, which has the
    benefits of both databases, but allowing sequencing to be performed using
    ...
    analyses."""
    scaffold_extensions_into_foundation(extension_trees_otu_map,
